...
<template><div class="user-list" data-test-user-list ...attributes>
  <div class="user-list-selected-users">
    <button type="button" class={{if (eq this.selectedUsers.length 0) "disabled"}} {{on "click" this.deselectAllUsers}}>
  ...
  <table>
    ...
    <tbody>
      {{#each @users as |user|}}
        <UserListItem @user={{user}} @selected={{this.isSelected user}} @onSelectUser={{this.selectUser}} @kloutLevel={{this.kloutLevel}} @onBuffKlout={{this.buffKlout}} @linkToUser={{true}} />
      {{/each}}
    </tbody>
  </table>
</div></template>}
